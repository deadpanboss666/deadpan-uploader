...
def split_sentences(text: str, max_chars: int = 110) -> List[str]:
    """
    Split text into punchy segments:
    - sentence-ish splitting with fallback
    - enforce max chars by further chunking on commas/spaces
    """
    t = re.sub(r"\s+", " ", text).strip()
    if not t:
        return []

    # Primary split: sentence end punctuation.
    ...
    # Final pass: hard wrap by words if still too long.
    out: List[str] = []
    for p in refined:
        if len(p) <= max_chars:
            out.append(p)
...
def ffprobe_duration_seconds(audio_path: Path) -> float:
    """
    Reliable duration from ffprobe.
    """
    out = _run([
        "ffprobe", "-v", "error",
    ...
    try:
        ...
        raise RuntimeError(f"ffprobe returned non-float duration for {audio_path}: {out}")
...
def concat_audio_mp3(inputs: List[Path], output: Path) -> None:
    ...
    tmp_list = output.parent / "concat_list.txt"
...
def build_segments_from_phrase_audio(
    ...
    """
    Build (start,end) timeline by summing real audio durations.
    Small gap helps readability and prevents overlaps if durations are tight.
    """
...
def escape_ass(text: str) -> str:
    """
    ASS escaping and line breaks control.
    """
    # Basic escapes
...
def write_ass_subtitles(
    ...
    """
    High-quality Shorts-ready ASS with:
    ...
    # Colors in ASS are BBGGRR (&HAABBGGRR for alpha)
    # BackColour uses alpha in the highest byte; &HAA000000 means black with alpha.
    # Lower AA = more opaque? Actually in ASS alpha: 00 is opaque, FF is transparent.
    back = "&H80000000"  # 50% transparent black
...
"""
    ...
    for seg in segments:
        ...
        # Force max 2 lines by inserting \N if long:
        txt = escape_ass(seg.text)

        # Heuristic split to 2 lines
        if len(txt) > 55 and " " in txt:
            words = txt.split(" ")
            mid = len(words) // 2
            # Try to balance line length
            best_i = mid
            best_score = 10**9
        ...
        # Add slight fade in/out for polish
        # \fad(in_ms,out_ms)
...
def generate_gtts_phrase_audio(
    ...
    """
    Generate one mp3 per phrase using gTTS.
    NOTE: requires gTTS installed. Network call occurs during generation.
    """
    ...
    for i, txt in enumerate(phrases):
        ...
        tts = gTTS(text=txt, lang=lang, tld=tld, slow=False)
        tts.save(str(p))
        paths.append(p)
    return paths
...
def build_voice_and_subs_from_text(
    ...
    """
    One-call helper:
    - split story into phrases
    - generate gTTS mp3 per phrase
    - concat into voice.mp3
    - compute real timestamps
    - write subtitles.ass
    """
